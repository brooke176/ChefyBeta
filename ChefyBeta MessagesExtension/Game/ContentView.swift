...
protocol GameLaunchDelegate: AnyObject {
...
struct ContentView: View {
    ...
    @State private var showingProfile = false
    @State private var showingSettings = false
    weak var gameLaunchDelegate: GameLaunchDelegate?

    init(conversation: MSConversation?, delegate: GameLaunchDelegate?) {
        self.conversationManager = ConversationManager(conversation: conversation)
                ...
                VStack {
                    ...
                    LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible()), GridItem(.flexible()), GridItem(.flexible())], spacing: 10) {
                        ForEach(imageItems, id: \.imageName) { item in
                            Button(action: {
                                selectedItem = item
                                gameLaunchDelegate?.launchGame(for: item) // ⬅️ Launch directly
                            }) {
                                itemContent(for: item)
                            }
                        }
